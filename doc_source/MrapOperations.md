...
Amazon S3 Multi\-Region Access Point Amazon Resource Names \(ARNs\) allow applications \(using an AWS SDK\) that require an Amazon S3 bucket name to use a Multi\-Region Access Point\. To check with AWS SDKs are compatible with Multi\-Region Access Points, see [Compatibility with AWS SDKs](https://docs.aws.amazon.com/sdkref/latest/guide/feature-s3-mrap.html#s3-mrap-sdk-compat)\.
...
To download objects from a Multi\-Region Access Point that is connected to a Requester Pays bucket, you must specify `--request-payer requester` as part of your [get\-object](https://docs.aws.amazon.com/cli/latest/reference/s3api/get-object.html) request\. You must also specify the name of the file in the bucket, as well as the location where it should be stored\.
