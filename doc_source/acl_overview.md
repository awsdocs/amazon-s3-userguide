...
# Access control list \(ACL\) overview<a name="acl_overview"></a>
...
**Warning**  
Use caution when granting access permissions to your S3 buckets and objects\. For example, granting `WRITE` access to a bucket allows the grantee to create, overwrite, and delete any object in the bucket\. We highly recommend that you read through this entire [Managing access with ACLs](acl-overview.md) section before granting permissions\.
