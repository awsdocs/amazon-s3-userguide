...
This example shows *version 2\.2* of the event notification JSON structure\. Amazon S3 uses *versions 2\.1* and *2\.2* of this event structure: version 2\.2 is used for cross\-region replication event notifications that contain extra information specific to these operations, but is otherwise compatible with version 2\.1 currently used for other operation types\.
